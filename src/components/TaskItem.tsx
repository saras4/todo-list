...
export default function TaskItem({ task, onToggle, onEdit, onDelete }: Props) {
  ...
  // After exit animation ends, invoke onDelete
  useEffect(() => {
  ...
  return (
    <Presence present={isVisible}>
      <div
        ref={itemRef}
        role="taskitem"
        className={`group flex items-center gap-3 rounded-md border border-transparent px-3 py-2 transition-colors hover:border-gray-200 dark:hover:border-gray-800 task-item task-enter ${
          !isVisible ? "task-exit" : ""
        }`}
